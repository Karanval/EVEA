...
class Manager {
  ...
  beforeCreate(newItemData) {
    let creationOptions = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  ...
  _parseOutputItem(item) {
    return new Promise((resolve, reject) => {
      ...
      this.parseResultItem(item).then(function (itemChanged) {
        resolve(itemChanged);
  ...
  _create(newItemData, model, resolve, reject) {
    let options = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : {};


    model = model || this.model;

    Promise.resolve(this.beforeCreate(newItemData, options)).then(curatedNewItemData => {

      return model.create(curatedNewItemData, options).then(createdItem => {

        return Promise.resolve(this.afterCreate(createdItem, curatedNewItemData)).then(curatedNewItem => {

          let idKey = model.primaryKeyAttributes[0];
          curatedNewItem.id = curatedNewItem[idKey];

          return this._parseOutputItem(curatedNewItem).then(curatedNewItemChanged => {
  ...
  update(id, updatedItemData) {
    return new Promise((resolve, reject) => {
      ...
      promise.then(item => {
        ...
        Promise.resolve(this.beforeUpdate(updatedItemData, item)).then(curatedUpdatedItemData => {
          ...
          item.update(curatedUpdatedItemData, {
            ...
            Promise.resolve(this.afterUpdate(updatedItem, originalData)).then(curatedUpdatedItem => {

              return this._parseOutputItem(curatedUpdatedItem).then(curatedUpdatedItemChanged => {
    ...
    return new Promise((resolve, reject) => {
      ...
      promise.then(item => {
        ...
        return item.destroy().then(() => {
          ...
          return this._parseOutputItem(item).then(curatedDeletedItem => {
  ...
  findByPk(id) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!findOptions.attributes && (options.include || []).length === 0) {
        promise = this.model.scope('detail', {
      ...
      return promise.then(item => {
        return this._parseOutputItem(item).then(function (itemChanged) {
          resolve(itemChanged);
        });
      }).catch(reject);
    });
  ...
  parseFilters() {
    let findOptions = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
  ...
  parseFilter() {
    let findOptions = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    let filterName = arguments[1];
    let filterContent = arguments[2];
  ...
  findAll() {
    ...
    return new Promise((resolve, reject) => {
      ...
      let findOptions = Object.assign({
        subQuery: true,
        order: this.order || [[this.model.primaryKeyAttributes[0], 'DESC']]
      }, options);
      ...
      if ((options.include || []).length === 0 && !findOptions.attributes) {
        ...
        promise = this.model.scope('summary', {
      ...
      return promise.then(collection => {
        ...
        return Promise.all(parseOutputItems).then(function (collectionChanged) {
          Promise.resolve(self.afterFindAll(collectionChanged, options)).then(result => {
  ...
  count() {
    ...
    return new Promise((resolve, reject) => {
      ...
      return this.model.count(findOptions).then(count => {
  ...
  static createSequelizeError(sequelizeError) {
    ...
    if (typeof sequelizeError === 'string') {
      return Manager.createError('ValidationError', sequelizeError);
    } else if (sequelizeError.errors) {
      return Manager.createError('ValidationError', sequelizeError.errors.map(error => {
        return error.message;
      }));
    } else if (sequelizeError.code) {
      return sequelizeError;
    } else {
      return Manager.createError('ValidationError', [sequelizeError.message]);
    }
  }
